...
L_SCOPE = ["https://spreadsheets.google.com/feeds",
           "https://www.googleapis.com/auth/spreadsheets",
           "https://www.googleapis.com/auth/drive.file",
           "https://www.googleapis.com/auth/drive"]

L_DB = "대한사료_리더대화_DB"
CAL_NAMES = ["리더와의 대화", "멘토링&코칭"]


class LeaderBusy(Exception):
    """구글 시트가 잠깐 응답하지 않을 때."""
    pass


def _l_retry(fn, *a, **kw):
    ...
    for i in range(3):
        try:
            ...
            if i >= 1:
                raise
            time.sleep(1.0)
    raise LeaderBusy(str(last))


@st.cache_resource(show_spinner=False)
def init_gspread_leader():
    creds = ServiceAccountCredentials.from_json_keyfile_dict(
        st.secrets["gcp_service_account"], L_SCOPE)
    client = gspread.authorize(creds)
    return _l_retry(client.open, L_DB)
...
def get_sheet_data_leader(sheet_name):
    ...
    ws = _l_retry(doc.worksheet, sheet_name)
    return _l_retry(ws.get_all_records)


def _reset_leader_conn():
    for f in (init_gspread_leader, get_sheet_data_leader, get_dooray_calendar_leader):
        try:
            f.clear()
        except Exception:
            pass
...
#      이제는 서버가 켜져 있는 동안 한 번만 접속하고 그대로 다시 씁니다.
# =========================================================
@st.cache_resource(show_spinner=False)
def get_dooray_calendar_leader():
    ...
    for test_url in urls_to_try:
        try:
            ...
            if calendars:
                for c in calendars:
                    if hasattr(c, 'name') and c.name in CAL_NAMES:
                        return c
                return calendars[0]
        except Exception:
            continue
...
@st.cache_resource(show_spinner=False)
def _l_cleanup_marker():
    """'오늘 이미 캘린더 청소를 했는지'를 서버 전체가 함께 기억합니다.
       (예전에는 접속한 사람마다 따로 기억해서, 사람마다 한 번씩 느렸습니다)"""
    return {}


def add_dooray_calendar_event_leader(name, date_obj, start_time, end_time, location,
                                     prefix="[예약가능]"):
    try:
...
def send_telegram_noti_leader(name, date, start, end, location):
    try:
...
def run_leader_talk():
    ...
    try:
        ...
        code = getattr(getattr(e, "response", None), "status_code", None)
        st.markdown("### ☕ 리더와의 대화")
        if code in (429, 500, 502, 503) or isinstance(e, LeaderBusy):
        ...
        if st.button("🔄 다시 시도", key="l_retry_btn", type="primary"):
            _reset_leader_conn()
            st.rerun()
        return

    # ⚡ 캘린더 청소는 화면을 다 그린 '뒤에' 조용히 합니다.
    #    (예전에는 화면을 그리기 '전에' 해서 몇 초씩 빈 화면이 보였습니다)
    auto_cleanup_past_dooray_events_leader()


def _run_leader_talk():
    st.markdown("""
        <style>
    ...
    def safe_save_leader(ws_name, data_list):
        try:
            doc = init_gspread_leader()
            ws = _l_retry(doc.worksheet, ws_name)
            _l_retry(ws.clear)
            if data_list:
                ...
                df = df.fillna("")
                _l_retry(ws.update, values=[df.columns.values.tolist()] + df.values.tolist())
            return True
        except Exception as e:
    ...
    with main_tab_mentee:
        ...
        if st.button("🔄 최신 현황 불러오기", key="l_refresh_1"):
        ...
        with sub_tab_schedule:
            ...
            if l_name_1 and l_pw_1:
                ...
                if not minfo:
                    ...
                    dv, sv, ev, lv = (c2_1.date_input("날짜", key="l_sd_t2"),
                                      c2_2.time_input("시작", datetime.time(0, 0), key="l_ss_t2"),
                                      c2_3.time_input("종료", datetime.time(0, 0), key="l_se_t2"),
                                      c2_4.text_input("장소", key="l_sl_t2"))
                        ...
                        if is_duplicate:
                            ...
                            with st.status("📡 저장 중..."):
                                ...
                                if safe_save_leader("slots", st.session_state.l_available_slots):
                                    ...
                                    is_noti_success = send_telegram_noti_leader(l_name_1, dv, sv, ev, lv)
    ...
    with main_tab_admin:
        ...
        if not st.session_state.l_admin_logged_in:
            ...
            if col_out.button("로그아웃", key="l_logout_btn"):
                st.session_state.l_admin_logged_in = False; st.rerun()
            if col_ref.button("🔄 연결 새로고침", key="l_reset_conn",
                              help="구글 시트나 캘린더가 말썽일 때 눌러 주세요."):
                _reset_leader_conn(); st.success("연결을 새로 읽었습니다."); st.rerun()
