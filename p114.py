...
# 서버가 직접 잰 시간이 이보다 짧으면 사람이 칠 수 없는 속도입니다.
#   (자판 723번 ÷ 30초 = 분당 1,446타. 국내 최상위권이 900타 수준입니다)
#   이 시간은 브라우저에서 고칠 수 없어서 가장 확실한 증거가 됩니다.
SERVER_MIN_SEC = 30


def _p_verdict(auto, paste, keys, sec=0.0, server_sec=0.0):
    ...
    # 화면이 보내온 기록과, 서버가 직접 잰 시간을 맞춰 봅니다.
    # (서버 시간은 화면에서 고칠 수 없으므로 시간 조작을 잡아냅니다)
    if 0 < server_sec < SERVER_MIN_SEC:
        return "⚠️ 부정 의심", ("서버 기준 %.0f초 만에 끝냈습니다 — 사람이 칠 수 없는 속도입니다"
                            % server_sec)
    if server_sec >= 20 and sec > 0 and sec < server_sec * 0.12:
        return "⚠️ 확인 필요", ("화면 기록 %.2f초 · 서버 측정 %.0f초 — 차이가 너무 큽니다 "
                            "(도중에 자리를 비운 경우일 수도 있습니다)" % (sec, server_sec))
    return "✅ 정상", ""
...
def save_p114_attempt(user, score, auto=0, paste=0, keys=0, server_sec=0.0):
    ...
    try:
        ...
        verdict, why = _p_verdict(auto, paste, keys, sec, server_sec)
    ...
    if True:
        ...
        if wc2.button("로그아웃", key="p_logout", use_container_width=True):
            for k in ('p_user', 'p_is_playing', 'p_step', 'p_hidden_time', 'p_score_saved',
                      'p_final_time', 'p_verdict', 'p_verdict_why', 'p_t_start', 'p_t_end'):
        ...
        if not st.session_state.p_is_playing:
            ...
            st.warning("🚫 **복사·붙여넣기와 자동완성(자동채우기)은 사용할 수 없습니다.** "
                       "시도하면 그 자리에서 막히고, **시도한 사실이 기록으로 남아 "
                       "관리자에게 전달됩니다.** 직접 입력한 기록만 인정됩니다.")

            if st.button("🚀 챌린지 시작하기", type="primary", use_container_width=True,
                         key="p_start_btn"):
                st.session_state.p_is_playing = True
                st.session_state.p_step = 0
                # 👇 서버가 직접 잰 시각입니다. 화면(브라우저)에서는 고칠 수 없습니다.
                st.session_state.p_t_start = time.time()
                st.session_state.pop("p_t_end", None)
                st.rerun()
            ...
            if not is_finished:
                ...
                if all(typed_all[i] == lines[i] for i in range(len(lines))):
                    ...
                    if st.session_state.p_step >= len(P114_STEPS):
                        st.session_state.p_t_end = time.time()
                    st.rerun()
                ...
                if js_time and 'p_score_saved' not in st.session_state:
                    ...
                    # 서버가 직접 잰 시간 (시작 버튼 → 마지막 줄 완료)
                    _ts = st.session_state.get("p_t_start")
                    _te = st.session_state.get("p_t_end")
                    server_sec = round(_te - _ts, 1) if (_ts and _te and _te > _ts) else 0.0
                    verdict, why = _p_verdict(n_auto, n_paste, n_keys,
                                              final_time_float, server_sec)
                    ...
                    if st.button("🔄 처음부터 다시 도전하기", key="p_again_btn"):
                        ...
                        for k in list(st.session_state.keys()):
                            if str(k).startswith("p_in_"):
                                del st.session_state[k]
                        for k in ('p_hidden_time', 'p_score_saved', 'p_final_time',
                                  'p_verdict', 'p_verdict_why', 'p_t_start', 'p_t_end'):
                            if k in st.session_state:
                                del st.session_state[k]
                        st.rerun()
